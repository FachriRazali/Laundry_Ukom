...
<div class="container futuristic-container">
    <div class="header">
        <h1 class="title"><i class="fas fa-users"></i> Manage Users</h1>
        <div>
            <a href="{{ route('dashboard') }}" class="btn-glass shadow-neon">
                <i class="fas fa-arrow-left"></i> Back to Dashboard
            </a>
            <a href="{{ route('users.create') }}" class="btn-glass btn-neon-green">
                    ...
                    <tbody>
                        @foreach ($users as $user)
                            <tr>
                                ...
                                <td>{{ $user->email }}</td>
                                <td><span class="badge role-badge">{{ ucfirst($user->role) }}</span></td>
                                <td class="text-center">
                                    <a href="{{ route('users.edit', $user->id) }}" class="btn-glass btn-neon-blue">
                                    ...
                                    <form action="{{ route('users.destroy', $user->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this user?');">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="btn-glass btn-neon-red">
...
<style>
    ...
    .btn-neon-blue {
        ...
        box-shadow: 0px 0px 15px rgba(0, 122, 255, 0.6);
    }
